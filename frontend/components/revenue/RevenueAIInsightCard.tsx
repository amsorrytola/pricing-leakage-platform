...
export default function RevenueAIInsightCard({
  contractId,
  period,
}: Props) {
  ...
  if (loading) {
    return (
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-5">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 animate-pulse items-center justify-center rounded-lg bg-gradient-to-br from-purple-600 to-indigo-600 shadow-sm">
              <svg
        ...
        <div className="p-6">
          <div className="flex items-center justify-center py-12">
            <div className="relative">
              ...
              <div className="absolute inset-0 h-12 w-12 animate-spin rounded-full border-4 border-purple-600 border-t-transparent" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-xs text-slate-500">
            Submit volume data to generate AI-powered insights
          </p>
        </div>
      </div>
    );
      ...
      <div className="border-b border-slate-200 bg-gradient-to-r from-purple-50 to-indigo-50 px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              {insights.length} intelligent analysis{" "}
              {insights.length === 1 ? "card" : "cards"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-h-[600px] space-y-4 overflow-y-auto p-6">
        {insights.map((insight) => {
          ...
          const recommendations =
            insight.recommendations?.next_steps || [];

          return (
            <div
              key={insight.id}
              className={`overflow-hidden rounded-xl border ${config.border} ${config.bg} transition-all hover:shadow-md`}
            >
              <div className="p-5">
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="flex min-w-0 flex-1 items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3
                        className={`mb-1.5 text-base font-semibold ${config.text}`}
                      >
                        {insight.title}
                      </h3>
                      <p className="text-xs text-slate-500">
                ...
                {isExpanded && (
                  <div className="mt-4 space-y-4 border-t border-slate-200 pt-4">
                    {recommendations.length > 0 && (
                      <div>
                        <h4 className="mb-2.5 text-xs font-semibold uppercase tracking-wide text-slate-700">
                          Recommendations
                        </h4>
                    ...
                    <div className="flex items-center gap-2 pt-2">
                      <span className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 py-1">
                        <svg
