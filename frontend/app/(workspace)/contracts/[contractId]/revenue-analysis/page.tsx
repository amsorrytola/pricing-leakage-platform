...
export default function RevenueAnalysisPage() {
  ...
  useEffect(() => {
    ...
    async function loadServices() {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/api/contracts/${institutionId}/billable-services-by-institution`
      ).then((r) => r.json());

      // res.services = [{ service_id: { unit, price, service_code } }, ...]

      const rows: ServiceRow[] = (res.services || []).map(
        (entry: Record<string, any>) => {
          const serviceId = Object.keys(entry)[0];
      ...
      console.log("Loaded billable services:", rows);
  ...
  useEffect(() => {
    ...
    async function loadExistingVolumeReport() {
      ...
      console.log("Loaded existing volume report:", volumeMap);
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">
            Volume-based revenue impact and leakage intelligence
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Enter transaction volumes to generate revenue impact analysis
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-fixed">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide w-[35%]">
                      Service Code
                    </th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide w-[15%]">
                      Unit
                    </th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide w-[20%]">
                      Price
                    </th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-slate-700 uppercase tracking-wide w-[30%]">
                      Volume
                    </th>
                  </tr>
                </thead>
                  ...
                  {services.map((s, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <input
                          type="number"
                          className="w-32 px-3 py-2 rounded-lg border border-slate-300
                  focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20
                  outline-none transition-all text-sm text-slate-900 text-right"
                          value={s.volume ?? ""}
                          onChange={(e) =>
                            updateVolume(i, Number(e.target.value))
                          }
                          placeholder="0"
                        />
                      </td>
