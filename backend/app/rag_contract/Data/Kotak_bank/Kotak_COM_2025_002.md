...
**Agreement ID:** Kotak/COM/2025/002
**Bank:** Kotak Mahindra Bank Limited
**Client:** Mindtree Limited
**Registration:** CIN: L72200KA2000PLC027287
**Effective Date:** February 4, 2025
**Expiry Date:** February 04, 2027
**Duration:** 2 Year(s)
**Total Pages:** 25-27
...
**CLIENT:** Mindtree Limited, Bangalore, India (Registration: CIN: L72200KA2000PLC027287)
...
This binding legal contract governs all banking transactions and services provided by Kotak Mahindra Bank Limited to Mindtree Limited including but not limited to:
...
The Bank maintains separate current accounts for Mindtree Limited in the core banking system with unique account numbers. Each account operates independently with separate transaction recording, balance maintenance, and statement generation.
...
**ACH Processing:** Mindtree Limited submits ACH file containing payment instructions in prescribed format (.NACH or CSV) with beneficiary details. The Bank validates the file, verifies IFSC codes, checks for duplicates, and submits to RBI ACH. Transactions settle through clearing house with settlement confirmation provided.

**RTGS Processing:** For transactions exceeding INR 2,00,000, Mindtree Limited initiates RTGS through online portal or branch. The Bank validates beneficiary details, prepares RTGS message, submits to RBI RTGS system for real-time settlement with confirmation within 5-10 minutes.
...
**Standing Instructions:** Mindtree Limited authorizes recurring payments to beneficiaries on predetermined dates. Bank automatically initiates payments on scheduled dates without requiring separate authorization for each transaction.

### 2.3 International Payment Services and Cross-Border Transfers

**SWIFT Wire Transfers:** Mindtree Limited provides remittance details including beneficiary name, SWIFT code, account number, amount, and currency. Bank verifies details for FATCA compliance, prepares SWIFT message in MT103 format, transmits through SWIFT network, and provides settlement confirmation.
...
**Correspondent Charges:** Bank recovers charges levied by correspondent banks in destination countries, either deducting from remittance amount or charging separately based on Mindtree Limited's instruction.

### 2.4 Trade Finance Services and Documentary Credits

**Letter of Credit Issuance:** Mindtree Limited applies for LC with beneficiary, amount, validity, and required documents. Bank verifies creditworthiness, prepares LC document in UCP 600 format, and transmits via SWIFT to advising bank.

**LC Amendment:** If Mindtree Limited requests LC amendment during validity (increasing amount, extending validity, modifying documents), Bank prepares amendment document and transmits via SWIFT MT707 to advising bank.
...
**Bank Guarantee Issuance:** Mindtree Limited applies for guarantee with type, amount, validity, and claim conditions. Bank verifies creditworthiness, prepares guarantee in standard format, and transmits to beneficiary.
...
**API Integration:** Bank provides RESTful API integration enabling Mindtree Limited's ERP or treasury system to directly submit transactions. API uses OAuth 2.0 authentication, TLS 1.2+ encryption, and webhook callbacks for real-time status updates.

**Sweep Account Management:** Bank provides automated liquidity optimization with daily monitoring of current account balance against Mindtree Limited's defined thresholds, automatic sweep-in to term deposits for interest earning, and automatic sweep-out when funds needed.
...
- For Mindtree Limited, LC issuance fee REDUCED to ₹16,000 (from ₹18,000) for LCs exceeding ₹50 Lakhs
...
All fees are due within 15 days of invoice date. Bank is authorized to auto-debit fees from Mindtree Limited's primary operating account without separate authorization for each debit.

**Late Payment Interest:** If fees remain unpaid 15 days after invoice date, Mindtree Limited pays late payment interest at 12.0% per annum calculated daily from due date to payment date.
...
If Bank fails to meet SLA metrics for two consecutive calendar months, Mindtree Limited is entitled to service credit equal to 10% of total monthly service fees (account maintenance + portal + sweep fees). Service credits automatically applied to next invoice without requiring claim.
...
- Maintaining strict confidentiality of Mindtree Limited information

### 6.2 Client's Representations

Mindtree Limited represents and warrants that it is:
...
Except for breaches of confidentiality, unauthorized fund transfers due to Bank negligence, or willful misconduct, Bank's total cumulative liability for all claims arising from this Agreement is limited to total fees paid by Mindtree Limited during the 12 months preceding the claim.
...
Mindtree Limited shall indemnify and hold harmless Bank from claims arising from:
...
- Third-party claims related to Mindtree Limited's business operations
...
- Mindtree Limited becomes insolvent or enters bankruptcy proceedings
...
**IN WITNESS WHEREOF**, the parties execute this Agreement as of February 4, 2025:

**FOR Kotak Mahindra Bank Limited:**

Signature: _________________________ | Date: February 4, 2025  
Name: [Bank Official]  
Designation: [Title]
